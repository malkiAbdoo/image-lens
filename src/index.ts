...
export function focus(element: HTMLElement = body) {
...
export function zoomLens(zoomRatio = 1.5, element: HTMLElement = body) {
  ...
  element.addEventListener('mousemove', event => {
    ...
    lens.style.backgroundSize =
      containerRect.width * cx + 'px ' + containerRect.height * cy + 'px';
...
export function drawingMode(
  element: HTMLElement = body,
  options: context = { lineWidth: 5, color: '#000' }
) {
